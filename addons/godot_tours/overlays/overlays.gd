...
func highlight_signal_nodes_by_path(paths: Array[String], button_index := -1, do_center := true, play_flash := false) -> void:
	highlight_tree_items(
		interface.signals_dialog_tree,
		func(item: TreeItem) -> bool: return Utils.get_tree_item_path(item) in paths,
		button_index,
		do_center,
		play_flash,
...
func highlight_code(start: int, end := 0, caret := 0, do_center := true, play_flash := false) -> void:
	...
	var rect_getter := func() -> Rect2:
		...
		if rect_start.position != -Vector2i.ONE:
			if rect_end.position != -Vector2i.ONE:
				rect = code_editor.get_global_transform() * Rect2(rect_start.merge(rect_end))
			else:
				var last_visible_line : int
				for i in range(start, end):
					rect = code_editor.get_rect_at_line_column(i, 0)
					if rect.position == -Vector2.ONE:
						last_visible_line = i-1
						break
				
				var rect_last_visible_line = code_editor.get_rect_at_line_column(last_visible_line, 0)
				rect = code_editor.get_global_transform() * Rect2(rect_start.merge(rect_last_visible_line))
			
			rect.position.x = code_editor.global_position.x
			rect.size.x = code_editor.size.x
		else:
			var first_visible_line: int
			if rect_end.position != -Vector2i.ONE:
				for i in range(start, end):
					rect = code_editor.get_rect_at_line_column(i, 0)
					if rect.position != -Vector2.ONE:
				...
				var rect_first_visible_line = code_editor.get_rect_at_line_column(first_visible_line, 0)
				rect = code_editor.get_global_transform() * Rect2(rect_end.merge(rect_first_visible_line))
			else:
				var last_visible_line : int
				for i in range(start, end):
					rect = code_editor.get_rect_at_line_column(i, 0)
				...
				for j in range(first_visible_line, end):
					rect = code_editor.get_rect_at_line_column(j, 0)
					if rect.position == -Vector2.ONE:
						last_visible_line = j-1
						break
				...
				rect = code_editor.get_global_transform() * Rect2(rect_first_visible_line.merge(rect_last_visible_line))
			rect.position.x = code_editor.global_position.x
			rect.size.x = code_editor.size.x
...
func highlight_code_from_lines(script_code: String, from: String, to: String , caret := 0, do_center := true, play_flash := false) -> void:
	var lines: PackedStringArray = script_code.split("\n")
	var start_line: int; var end_line: int
		
	for i in range(lines.size()):
		if lines[i].strip_edges().begins_with(from):
			start_line = i+1
			break
		
	for j in range(start_line, lines.size()):
		if lines[j].strip_edges().begins_with(to):
			end_line = j+1
			break
		
	highlight_code(start_line, end_line, caret, do_center, play_flash)
