...
func get_checkpoint_res(res_path: String) -> Checkpoint:
	var res : Checkpoint
...
func save_checkpoint(res : Checkpoint)->void:
	ResourceSaver.save(res)


func load_checkpoint(path: String)->String:
...
func clear_output_console()->void:
	interface.run_bar_play_button.emit_signal('pressed')
	await get_tree().create_timer(1.0).timeout # dunno if necessary ¯\_(ツ)_/¯
	interface.run_bar_stop_button.emit_signal('pressed')
...
func scroll_script(line: int = 0 , duration := 1.0) -> void:
	queue_command(func () -> void:
		var scroll_script_editor : CodeEdit = EditorInterface.get_script_editor().get_current_editor().get_base_editor()
		var tween = EditorInterface.get_edited_scene_root().create_tween() # get_tree().create_tween only if inside queue_command()
		tween.tween_property(scroll_script_editor, "scroll_vertical", line, duration) 
	)  


func scroll_script_to_string(target_string: String, duration:= 1.0) -> void:
	queue_command(func()->void:
		var script_code: String = EditorInterface.get_edited_scene_root().get_script().source_code
		var lines: PackedStringArray = script_code.split("\n")
		var target_line: int
	
		for i in range(lines.size()):
			if lines[i].strip_edges().begins_with(target_string):
				target_line = i
				break
		
		var scroll_script_editor : CodeEdit = EditorInterface.get_script_editor().get_current_editor().get_base_editor()
		var tween = EditorInterface.get_edited_scene_root().create_tween() # get_tree().create_tween only if inside queue_command()
		tween.tween_property(scroll_script_editor, "scroll_vertical", target_line, duration)
	)
...
# for CheckBox type
func bubble_add_task_toggle_property(property_name : String, state := true) -> void:
	
	var description = ""
	if state == true: description = 'on' 
	else: description = 'off'
	
	bubble_add_task(
		gtr("Toggle [b]%s[/b] the %s property" % [description, property_name.capitalize()]), 1,
		func check_toggle_state(task: Task) -> int:
			var all_properties := interface.inspector_editor.find_children("", "EditorProperty", true, false)
			
			for property: EditorProperty in all_properties:
				if property.get_edited_property() == property_name:
					var checkbox : CheckBox = property.get_child(0)
					if checkbox.button_pressed == state: return 1 else: return 0	
			return 0,
	)
...
func bubble_add_task_rename_node(new_name: String , recursive: bool = true, owned: bool = true ):
	bubble_add_task(
		gtr("Rename the node to [b]%s[/b]" % new_name), 1,
		func find_renamed_node(task: Task) -> int:
			var renamed_node: Node = EditorInterface.get_edited_scene_root().find_child(new_name, recursive, owned)
			if renamed_node: return 1 else: return 0,
	)
...
func bubble_add_task_property_click(property_name: String, description := "") -> void:
	if description.is_empty():
		description = gtr("""Click on the property [b]%s[/b] in the [b]Inspector[/b]""") % property_name.capitalize()
	
	bubble_add_task(description, 1, func wait_for_click(_task: Task) -> int:
		var properties := interface.inspector_editor.find_children("", "EditorProperty", true, false)
		
		for property: EditorProperty in properties:
			if property.get_edited_property() == property_name:
				# Check if this property has received a mouse click
				if property.is_pressed():  # Hypothetical function, depends on implementation
					return 1  # Success
		return 0  # Not yet clicked
	)
...
func bubble_add_task_focus_node(node_name: String, description := "") -> void:
	...
	bubble_add_task(
		...
		func focus_camera_task(task: Task) -> int:
			...
			#return 1 if task.is_done() or is_node_selected else 0
			return 1 if is_node_selected else  0
	)
...
func highlight_signal_nodes_by_path(paths: Array[String], button_index := -1, do_center := true, play_flash := true) -> void:
	queue_command(overlays.highlight_signal_nodes_by_path, [paths, button_index, do_center, play_flash])
